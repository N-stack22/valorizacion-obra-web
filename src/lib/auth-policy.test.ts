...
describe("auth policy - caja blanca", () => {
  ...
  it("solo admin o residente pueden crear proyectos", () => {
    ...
    expect(canCreateProjectWithRoles({ appRoles: [], globalRoles: ["super_admin"] })).toBe(true);
    expect(canCreateProjectWithRoles({ appRoles: [], globalRoles: ["admin_empresa"] })).toBe(true);
