...
function ExpedientePage() {
  ...
  const periodMemoria = useMemo(
    () =>
      periodMonth
        ? memorias.find((item) => item.project_id === projectId && item.period_month === periodMonth) ?? null
        : null,
  ...
  async function createPeriod(form: { number: number; from: string; to: string }): Promise<boolean> {
    ...
    if (!data) {
      toast.error("No se pudo recuperar el período creado. Actualiza la página e inténtalo nuevamente.");
      return false;
    }
  ...
  async function createPeriodMemoria() {
    ...
    const html = sections
      .map(([label, value]) => {
        ...
        if (!text) return "";
        return `<h3>${label}</h3><p>${text.replace(/</g, "&lt;").replace(/\n+/g, "</p><p>")}</p>`;
      })
      .filter(Boolean)
      .join("");
  ...
  async function addDeduction() {
    ...
    if (!data) return toast.error("No se pudo recuperar la deducción creada. Actualiza la página e inténtalo nuevamente.");
  ...
  async function generatePdf() {
    ...
    if (!periodMemoria) {
      toast.error("No se encontró la memoria del período. Crea el registro antes de generar el PDF.");
      return;
    }
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                  ...
                  <div className="rounded-md border p-3">
                    <p className="text-[11px] uppercase text-muted-foreground">Antecedentes / resumen del avance</p>
                    <Textarea
                    ...
                    />
                    <p className="mt-1 text-xs text-muted-foreground">La IA puede proponer este texto a partir de metrados y valorización.</p>
                  </div>
                </div>
            ...
            <CardContent className="space-y-3">
              {periodMemoria ? (
                ...
                <div className="flex flex-col gap-3 text-sm text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
                  <span>No se encontró la memoria del período. Créala para enviarla a revisión desde este asistente.</span>
                  <Button onClick={createPeriodMemoria} disabled={creatingMemoria}>
                    {creatingMemoria ? <Loader2 className="mr-1 h-4 w-4 animate-spin" /> : <Plus className="mr-1 h-4 w-4" />}
