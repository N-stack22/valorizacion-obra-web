...
type RawFormula = Omit<Formula, "monomios"> & {
  monomios: unknown;
};

type CalcTabProps = Readonly<{
  projectId: string;
  formulas: Formula[];
  indices: IneiIndex[];
  reajustes: Reajuste[];
...
type FormulasTabProps = Readonly<{
  projectId: string;
  formulas: Formula[];
  userId: string | undefined;
  onChange: () => Promise<void>;
...
type ImportCsvDialogProps = Readonly<{
  onChange: () => Promise<void>;
}>;

type IndicesTabProps = Readonly<{
  indices: IneiIndex[];
  isAdmin: boolean;
  onChange: () => Promise<void>;
}>;
...
function isMonomio(value: unknown): value is Monomio {
  return (
    isRecord(value) &&
    typeof value.symbol === "string" &&
    typeof value.index_code === "string" &&
    Number.isFinite(Number(value.coefficient)) &&
    Number.isFinite(Number(value.base_index_value))
  );
}

function normalizeFormula(row: RawFormula): Formula {
  return {
    ...row,
    monomios: Array.isArray(row.monomios) ? row.monomios.filter(isMonomio) : [],
  };
}
...
export function ReajustesPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    else setFormulas(((f.data ?? []) as RawFormula[]).map(normalizeFormula));
    ...
    <div className="space-y-4">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            K = Σ (coef × Ii / Ioi). Reajuste = Monto base × (K − 1).
          </CardDescription>
        </CardHeader>
          ...
          {calc ? (
            <div className="space-y-3 rounded-md border p-4">
              ...
              <div className="flex flex-wrap gap-4 text-sm">
                <div>
                  Σ coeficientes: <strong>{calc.totalCoeff.toFixed(3)}</strong>
                </div>
                <div>
                  K: <strong>{calc.k.toFixed(6)}</strong>
                </div>
                <div>
                  Reajuste: <strong>{formatMoney(calc.reajuste)}</strong>
                </div>
              </div>
              {Math.abs(calc.totalCoeff - 1) > 0.001 ? (
                <p className="text-xs text-destructive">
                  Advertencia: la suma de coeficientes debe ser 1.000.
                </p>
              ) : null}
              <Button onClick={save} disabled={saving || !formula || !periodMonth}>
                <Calculator className="mr-1 h-4 w-4" />{" "}
                {saving ? "Guardando…" : "Registrar reajuste"}
              </Button>
            </div>
          ) : null}
        </CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Historial de reajustes</CardTitle>
        </CardHeader>
        <CardContent>
          {reajustes.length === 0 ? (
              ...
              <TableBody>
                {reajustes.map((r) => (
                  <TableRow key={r.id}>
                    ...
                    <TableCell>
                      {formulas.find((f) => f.id === r.formula_id)?.name ?? "—"}
                    </TableCell>
                    <TableCell>
                      {r.valuation_id ? (
...
// ============== FÓRMULAS ==============
function FormulasTab({ projectId, formulas, userId, onChange }: FormulasTabProps) {
  ...
  const addMon = () =>
    setMonomios((m) => [...m, { symbol: "", coefficient: 0, index_code: "", base_index_value: 0 }]);
  ...
  const save = async () => {
    ...
    const { error } = await supabase.from("polynomial_formulas").insert({
      ...
      monomios: monomios as unknown as Json,
      created_by: userId,
    });
    setSaving(false);
    if (error) {
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          ...
          <CardDescription>
            Cada monomio = símbolo · coeficiente · índice INEI · valor base (Ioi).
          </CardDescription>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button>
              <Plus className="mr-1 h-4 w-4" /> Nueva fórmula
            </Button>
          </DialogTrigger>
          <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Define los monomios. La suma de coeficientes debe ser 1.000.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-3">
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    type="month"
                    value={basePeriod ? basePeriod.slice(0, 7) : ""}
                    onChange={(e) => setBasePeriod(e.target.value ? `${e.target.value}-01` : "")}
                  />
                </div>
              </div>
              <Table>
                  ...
                  {monomios.map((m, i) => (
                    <TableRow key={i}>
                      <TableCell>
                        <Input
                          value={m.symbol}
                          onChange={(e) => updMon(i, { symbol: e.target.value })}
                        />
                      </TableCell>
                      <TableCell>
                        <Input
                          ...
                          step="0.001"
                          value={m.coefficient}
                          onChange={(e) => updMon(i, { coefficient: Number(e.target.value) })}
                        />
                      </TableCell>
                      <TableCell>
                        <Input
                          value={m.index_code}
                          onChange={(e) => updMon(i, { index_code: e.target.value })}
                          placeholder="39"
                        />
                      </TableCell>
                      <TableCell>
                        <Input
                          type="number"
                          step="0.0001"
                          value={m.base_index_value}
                          onChange={(e) => updMon(i, { base_index_value: Number(e.target.value) })}
                        />
                      </TableCell>
                      <TableCell>
                        <Button variant="ghost" size="icon" onClick={() => rmMon(i)}>
              ...
              <div className="flex items-center justify-between">
                <Button variant="outline" size="sm" onClick={addMon}>
                  <Plus className="mr-1 h-4 w-4" /> Agregar monomio
                </Button>
                <span
                  className={`text-xs ${Math.abs(total - 1) > 0.001 ? "text-destructive" : "text-muted-foreground"}`}
                >
                  Σ = {total.toFixed(3)}
                </span>
              </div>
            ...
            <DialogFooter>
              <Button onClick={save} disabled={saving || !name || !basePeriod}>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>Mes base</TableHead>
                <TableHead>Monomios</TableHead>
                <TableHead />
              </TableRow>
            </TableHeader>
            <TableBody>
              {formulas.map((f) => (
                <TableRow key={f.id}>
                  ...
                  <TableCell className="text-xs">
                    {f.monomios
                      .map((m) => `${m.symbol}=${m.coefficient}·I${m.index_code}`)
                      .join(" + ")}
                  </TableCell>
                  <TableCell>
...
function parseIndicesCsv(text: string, fallbackPeriod: string | null): ParseResult {
  ...
  if (matrix.length === 0) {
    return {
      rows: [],
      errors: ["El archivo está vacío o solo contiene comentarios."],
      warnings,
      unknownColumns,
      missingColumns,
    };
  ...
  header.forEach((h, idx) => {
    ...
    if (!canon) {
      unknownColumns.push(matrix[0][idx]);
      return;
    }
    if (seenCanon.has(canon)) {
      warnings.push(
        `Columna "${matrix[0][idx]}" duplicada — se usará la primera ocurrencia de "${canon}".`,
      );
      return;
    }
    seenCanon.add(canon);
  ...
  if (missingColumns.length > 0) {
    errors.push(
      `Faltan columnas obligatorias: ${missingColumns.join(", ")}. Cabecera esperada: ${TEMPLATE_COLUMNS.join(",")}.`,
    );
  ...
  for (let i = 1; i < matrix.length; i++) {
    ...
    if (r.length !== expectedCols) {
      rowErrors.push({
        field: "_row",
        message: `Número de columnas inesperado: ${r.length} (se esperaban ${expectedCols}).`,
      });
    }

    const rawPeriod =
      headerMap.period_month !== undefined ? (r[headerMap.period_month] ?? "").trim() : "";
    ...
    if (rawPeriod) {
      ...
      if (!period)
        rowErrors.push({
          field: "period_month",
          message: `Mes inválido: "${rawPeriod}". Usa YYYY-MM o YYYY-MM-DD.`,
        });
    ...
    if (!code) rowErrors.push({ field: "code", message: "Código vacío." });
      ...
      rowErrors.push({ field: "code", message: `Código demasiado largo (${code.length}>32).` });
    else if (!/^[A-Za-z0-9._-]+$/.test(code))
      rowErrors.push({ field: "code", message: `Caracteres inválidos en código: "${code}".` });

    const description =
      headerMap.description !== undefined ? (r[headerMap.description] ?? "").trim() || null : null;
    if (description && description.length > 255) {
      rowErrors.push({
        field: "description",
        message: `Descripción demasiado larga (${description.length}>255).`,
      });
    }
    ...
    if (!rawValue) {
      ...
      if (!/^-?\d+(\.\d+)?$/.test(normalized)) {
        ...
        if (!Number.isFinite(value))
          rowErrors.push({ field: "value", message: `Valor fuera de rango: "${rawValue}".` });
        else if (value <= 0)
          rowErrors.push({ field: "value", message: `Valor debe ser mayor que 0 (${value}).` });
        else if (value > 100000)
          rowErrors.push({ field: "value", message: `Valor sospechosamente alto (${value}).` });
      }
    }
    ...
    if (
      period &&
      code &&
      !rowErrors.some((e) => e.field === "period_month" || e.field === "code")
    ) {
      ...
      if (prev !== undefined) {
        rowErrors.push({
          field: "_row",
          message: `Duplicado intra-archivo de la línea ${prev} (mismo mes y código).`,
        });
      } else {
...
function ImportCsvDialog({ onChange }: ImportCsvDialogProps) {
  ...
  const reparseWithPeriod = (p: string) => {
    ...
    file
      .text()
      .then((text) => applyParse(text, p || null))
      .catch(() => undefined);
  };
  ...
  const downloadErrorReport = () => {
    ...
    for (const r of invalid) {
      for (const e of r.__errors) {
        ...
        lines.push(
          `${r.__line},${e.field},${msg},${r.period_month},${r.code},${Number.isFinite(r.value) ? r.value : ""}`,
        );
      }
    }
    const blob = new Blob([lines.join("\n")], { type: "text/csv;charset=utf-8" });
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = "errores-importacion-indices.csv";
    a.click();
    URL.revokeObjectURL(url);
  };
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!result.ok) {
        const firstErrs = result.errors
          .slice(0, 3)
          .map((e) => `L${e.line} ${e.field}: ${e.message}`)
          .join(" | ");
        toast.error(`Backend rechazó ${result.errors.length} error(es)`, {
          description: firstErrs,
        });
        setSaving(false);
        return;
      }
  ...
  const fieldError = (r: ParsedRow, field: string) =>
    r.__errors.find((e) => e.field === field)?.message;
    ...
    >
      ...
      <DialogContent className="max-h-[90vh] max-w-4xl overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Cabecera esperada: <code>period_month,code,description,value</code>. Solo{" "}
            <code>code</code> y <code>value</code> son obligatorias. Filas con (mes, código)
            duplicado actualizan el valor existente.
          </DialogDescription>
        </DialogHeader>
          ...
          {parsed ? (
            <div className="space-y-2">
              <div className="flex flex-wrap items-center gap-2 text-sm">
                ...
                {invalid.length > 0 ? (
                  <Badge variant="destructive">Inválidas: {invalid.length}</Badge>
                ) : null}
                {invalid.length > 0 ? (
                  <>
                    <Button
                      ...
                      onClick={() => setShowOnlyInvalid((v) => !v)}
                    >
                    ...
                    {displayRows.map((r, i) => {
                      ...
                      return (
                        <TableRow key={i}>
                          ...
                          <TableCell
                            className={`align-top ${cellClass(r, "period_month")}`}
                            title={fieldError(r, "period_month")}
                          >
                            {r.period_month || "—"}
                          </TableCell>
                          <TableCell
                            className={`align-top ${cellClass(r, "code")}`}
                            title={fieldError(r, "code")}
                          >
                            {r.code || "—"}
                          </TableCell>
                          <TableCell
                            className={`max-w-[180px] truncate align-top ${cellClass(r, "description")}`}
                            title={fieldError(r, "description") ?? r.description ?? ""}
                          >
                            {r.description ?? "—"}
                          </TableCell>
                          <TableCell
                            className={`align-top ${cellClass(r, "value")}`}
                            title={fieldError(r, "value")}
                          >
                          ...
                          <TableCell className="align-top">
                            {r.__errors.length === 0 ? (
                              ...
                              <div className="space-y-1">
                                {rowErr ? (
                                  <div className="text-[10px] text-destructive">⚠ {rowErr}</div>
                                ) : null}
                                {fieldErrs.map((e, j) => (
...
function IndicesTab({ indices, isAdmin, onChange }: IndicesTabProps) {
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between gap-3">
        <div>
          ...
          <CardDescription>
            {isAdmin
              ? "Solo administradores globales pueden modificar."
              : "Consulta del catálogo INEI."}
          </CardDescription>
        </div>
      ...
      <CardContent className="space-y-4">
        {isAdmin ? (
          <div className="grid gap-3 rounded-md border p-3 md:grid-cols-5">
            ...
            <div className="md:col-span-5">
              <Button onClick={save} disabled={saving || !period || !code || !value}>
        ...
        <div className="space-y-1">
          ...
          <Input
            type="month"
            value={filterPeriod ? filterPeriod.slice(0, 7) : ""}
            onChange={(e) => setFilterPeriod(e.target.value ? `${e.target.value}-01` : "")}
            className="max-w-xs"
          />
        </div>
