...
interface AIDraftDialogProps {
  ...
  onApply?: (draft: DraftSections) => void;
  /**
...
function getAIToastMessage(message: string): string {
  if (message.includes("HTTP 429") || /rate.?limit|too many requests/i.test(message)) {
    return "Anthropic alcanzó el límite de uso. Espera unos minutos y vuelve a generar el borrador.";
  }
  if (message.includes("HTTP 402") || /credit|billing|payment/i.test(message)) {
    return "Anthropic no tiene créditos disponibles o requiere revisar la facturación de la cuenta.";
  }
  return message;
}
...
export function AIDraftDialog({
  ...
  async function runGeneration(targetSections?: DraftSectionKey[]) {
    ...
    try {
      ...
      if (!result.ok) {
        toast.error(getAIToastMessage(result.error));
        return;
      }
      ...
      toast.error(
        err instanceof Error ? getAIToastMessage(err.message) : "Error al generar el borrador.",
      );
    } finally {
      setLoading(false);
      setRegenKey(null);
  ...
  function applyAll() {
    const hasContent = DRAFT_SECTIONS.some((section) => (sections[section.key] ?? "").trim());
    if (!hasContent) {
    ...
    if (onApply) {
      onApply(sections);
      toast.success("Borrador aplicado al formulario. Revísalo antes de generar el documento.");
      setOpen(false);
      return;
    }
    if (!onApplyAll) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <Button
        ...
        size="sm"
        onClick={() => {
          setProjectId(fixedProjectId ?? defaultProjectId ?? projectId);
          setPeriod(((periodMonth ?? defaultPeriod)?.slice(0, 7) ?? period) || "");
